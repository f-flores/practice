...
module.exports = {
  ...
  findCustomersByFranchiseId: function (req, res) {
    const franchiseId = req.params.id;
    Franchise
		  // Find franchise by its id
		  .findById(franchiseId)
		  // Specify that we want to populate the retrieved franchise with associated customers
      .populate("customers")
			.then(function(franchiseData) {
				// If no data were found, send back a 404
				if (!franchiseData) res.status(404).end();

				// If able to successfully find and associate franchise and its customers, send back to client
				res.json(franchiseData);
			})
			.catch(function(err) {
				// If an error occurs, send it back to the client
  ...
  removeStaffMember: function(req, res) {
    const {email} = req.body;
    const franchiseId = req.params.id;
    let userId;
    console.log(`in franchiseController() removeStaffMember to be removed: ${email}`);
    User
    .find({email: email})
    .then(dbUser => {
      if (dbUser.length === 0) {
        res.statusMessage = "User not found in our system. Please try again"
        res.status(404).send(res.statusMessage);
      }
      const userFound = dbUser[0];
      console.log(`in franchiseController() remove user userFound: ${userFound} franchiseId: ${franchiseId}`);
      /* if (userFound.franchise.length > 0) {
        res.statusMessage = "Email belongs to other franchise";
        res.status(422).send(res.statusMessage);
      } else { */
        userId = userFound._id;
        return User.findOneAndUpdate({_id: userId}, {$pull: {franchise: franchiseId} }, { new: true });
      // }
    })
    .then(function(updatedUser) {
      if (updatedUser !== "undefined") {
        console.log(`removeStaffMember ${JSON.stringify(updatedUser)}`);
        // take away staff member from Franchise members array
        return Franchise.findOneAndUpdate({_id: franchiseId}, {$pull: {members: userId}}, {new: true});
      }
    })
    .then(function(updatedFranchise) {
      if (updatedFranchise !== "undefined") {
